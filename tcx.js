...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const rawSummaryTableBody = document.getElementById("rawSummaryTableBody");
    ...
    function calculateAverageSpeed(times, distances) {
    ...
    function calculateSegmentSpeeds(times, distances, minSpeedMph = 5.0) {
        const segmentSpeeds = [];
        if (times.length < 2 || distances.length < 2)
    ...
    function calculateTotalElevationChangeInFeet(altitudes) {
    ...
    function calculatePowerGroupTimes(times, powers) {
        const groupTimes = {};
        if (times.length < 2 || powers.length < 2)
            return groupTimes;
